 ...
 * segment.
 *
 * <code>Importer</code> is coded as a Hadoop job and is intended to
 ...
 * <code>Importer</code> is used to import a set of ARCs into a Nutch
...
public class Importer extends Configured implements Tool, Mapper<WritableComparable, Writable, Text, NutchWritable>
{
   ...
   * @param conf
   */
  public Importer( Configuration conf )
  {
    setConf( conf );
  }
   ...
   */
  public void map( final WritableComparable key, 
  ...
  /**
   * Normalize and filter the URL.  If the URL is malformed or
   ...
   * @param  digest of URL content
   * @param  date of URL capture
                       ...
                       Content         content )
  {
    ...
    // the Content is not added to the output.  But in Importer, we still add it, even
   ...
   */
  private byte[] readBytes( ARCRecord record, long contentLength )
    throws IOException
  {
  ...
  {
    int result = ToolRunner.run( NutchConfiguration.create(), new Importer(), args );
